...
export const addAgents = () => async (dispatch, getState) => {
  try {
    ...
    if (!getState().agent.agents.find((item) => item.name === current_agent.new_name)) {
      ...
      const payload = {
        agents: [
          ...
          { name: current_agent.new_name, agent: current_agent.agent },
        ],
        agents_names: [...getState().agent.agents_names, current_agent.new_name].sort(),
      };
      console.log(payload);
...
export const updateAgentName =
  () => async (dispatch, getState) => {
    try {
      const current_agent = getState().agent.current_agent;
      if (
        getState().agent.agents.find((item) => item.name === current_agent.name) &&
        current_agent.name !== current_agent.new_name &&
        !getState().agent.agents.find((item) => item.name === current_agent.new_name)
      ) {
...
export const deleteAgent = () => async (dispatch, getState) => {
  try {
    const current_agent = getState().agent.current_agent;
    if (getState().agent.agents.find((agent) => agent.name === current_agent.name)) {
